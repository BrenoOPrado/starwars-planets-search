...
import App from '../App';
import renderWithRouter from '../helpers/RenderWhithRouter';
...
describe('Testando o funcionamento da aplicação', () => {
  it('Teste da renderização básica dos componentes', () => {
    ...
    Object.keys(mockAPI.results[0]).forEach((word) => {
      screen.queryByRole('columnheader', { name: word });
    });

    mockAPI.results.forEach((planet) => {
      screen.queryByRole('cell', { name: planet.name });
      screen.queryByRole('cell', { name: planet.diameter });
      screen.queryByRole('cell', { name: planet.gravity });
      screen.queryByRole('cell', { name: planet.population });
      screen.queryByRole('cell', { name: planet.rotation_period});
      screen.queryByRole('cell', { name: planet.orbital_period });
      screen.queryByRole('cell', { name: planet.climate });
      screen.queryByRole('cell', { name: planet.terrain });
      screen.queryByRole('cell', { name: planet.surface_water });
    });

    const columnSelect = screen.getByRole('combobox', { name: /coluna:/i });
    const comparisonSelect = screen.getByRole('combobox', { name: /comparação:/i });
    const valueInput = screen.getByRole('spinbutton');
    const filterBtn = screen.getByRole('button', { name: /filtrar/i });
    const textValue = screen.getByRole('textbox');
    const removeFiltersBtn = screen.getByRole('button', { name: /remover filtros/i });

    userEvent.selectOptions(columnSelect, 'diameter');
    userEvent.selectOptions(comparisonSelect, 'menor que');
    userEvent.type(valueInput, '8901');
    ...
    screen.queryByRole('listitem', { name: 'diameter menor que 8901' });
    const deleteBtns = screen.queryAllByRole('button', { name: /deletar/i });
